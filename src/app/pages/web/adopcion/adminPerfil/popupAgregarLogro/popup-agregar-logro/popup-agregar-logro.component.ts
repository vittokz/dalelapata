...
@Component({
  selector: 'app-popup-agregar-logro',
  templateUrl: './popup-agregar-logro.component.html',
  styleUrls: ['./popup-agregar-logro.component.css']
})
export class PopupAgregarLogroComponent implements OnInit {
  dismissible = true;
...
dato: {
  ...
  constructor(private formBuild: FormBuilder,private ruta: Router,
    ...
    private rutaActiva: ActivatedRoute,private mascotaService: MascotasService) {
  ...
  cargarDatosMascota(){
      this.mascotaService.getIdMascota(this.dato.idMascota).subscribe(
        data=>{
          this.mascota = data;
          this.nombreMascota = this.mascota[0].nombre;
        }
      );
  }
