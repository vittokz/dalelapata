...
import {
  Fundacion,
  tipoFundacion,
} from "../../../../modelos/modulo-fundacion/fundacion-modelo";
...
export class RegistrarFundacionComponent implements OnInit {
  ...
  cargarTiposFundacion() {
    this.tipoFundacionService.getTipoFundaciones().subscribe((dataTipos) => {
  ...
  agregarFundacion() {
    ...
    this.fundacionService.uploadFile(formData).subscribe((data) => {
      if (data["resul"] > 0) {
        this.envioForm = "ok";
        this.registro.reset();
      }
