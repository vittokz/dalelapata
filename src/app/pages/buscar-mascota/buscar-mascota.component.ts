...
export class BuscarMascotaComponent implements OnInit {
  ...
  constructor(
    private userAuth: AuthUsuariosService,
  ...
  generarInfoMascota(dataMascota) {
    this.mascotaService
      .getPropietarioMascotaId(dataMascota.idMascota)
      .subscribe((propietario) => {
        this.propietario = propietario;
        this.generarWord(dataMascota, this.propietario);
      });
  }

  generarWord(mascota, propietario: PropietarioMascota[]) {
    console.log(propietario);
    let content =
      "<h3><center>INFORMACIÓN DE MASCOTA Y PROPIETARIO</center></h3><center>";
    content =
      content +
      "<span style='font-size:13px; font-weight:800'> Nombre: " +
      propietario[0].nombres +
      " " +
      propietario[0].apellidos +
      "</span><br>";
    content =
      content +
  ...
  cargarFundaciones() {
    this.fundacionService.getFundaciones().subscribe((data) => {
      this.fundaciones = data;
    });
  }

  cargarRazas() {
    this.mascotaService.getRazaAll().subscribe((data) => {
      this.razas = data;
    });
  }
  ...
  eliminar(mascotaSelec) {
    ...
    this.mascotaService.eliminarMascota(idMascota).subscribe((data) => {
      if (data["resul"] > 0) {
        this.eliminado = "ok";
        this.cargarMascotas();
      } else {
  ...
  editarMascota() {
    ...
    this.mascotaService
      .editarMascotaAdmin(this.mascotaModificar)
      .subscribe((data) => {
        if (data["resul"] == 1) {
          this.editado = "ok";
          //this.registro.reset();
