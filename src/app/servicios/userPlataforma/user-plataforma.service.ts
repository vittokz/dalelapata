...
import { UserPagina, UserPagAuth } from '../../modelos/modulo-usuario/usuarioPagina-modelo';
...
export class UserPlataformaService {
  ...
  actualizarUsuario(usuario: UserPagina): Observable<any>{
    ...
    options 	: any		= {
        ...
        return this.http.post(this.url + "actualizarUsuario.php", JSON.stringify(options), headers);
  }
  ...
  agregarUsuario(usuario: UserPagina): Observable<any>{
    ...
    options 	: any		= {
        ...
        return this.http.post(this.url + "addUsuario.php", JSON.stringify(options), headers);
  }

  //agregar usuario y postulacion

  agregarUsuarioyPostulacion(usuario: UserPagina, idMascota: string): Observable<any>{
    let headers 	: any		= new HttpHeaders({ 'Content-Type': 'application/json' }),
    options 	: any		= {
        "idMascota" : idMascota,
        "idFundacion": usuario.idFundacion,
    ...
    console.log(options);
        return this.http.post(this.url + "addUsuarioPostulacion.php", JSON.stringify(options), headers);
  }
  ...
  editarClaveUsuario(identidad:string,clave:string): Observable<any>{
    ...
    options 	: any		= {
       ...
       "clave": clave
        ...
        return this.http.post(this.url + "editarClaveUsuario.php", JSON.stringify(options), headers);
  }
 ...
 editarUsuario(usuario: UserPagina):Observable<any>{
  ...
  options 	: any		= {
      ...
      "twitter": usuario.twitter
       ...
       return this.http.post(this.url + "updateUsuario.php", JSON.stringify(options), headers);
}

  //traer Usuario mediante IdUsuario
  getUsuarioId(idUsuario: string):Observable<UserPagina>{
    return this.http.get<UserPagina>(this.url + 'getUsuarioId.php?idUsuario=' + idUsuario);
 }

 //traer Usuario mediante Identidad
 getUsuarioIdentidad(identidad: string):Observable<UserPagina>{
  return this.http.get<UserPagina>(this.url + 'getUsuarioIdentidadAdopcion.php?identidad=' + identidad);
}


 //traer Usuario mediante nomUsuario
 getUsuarioNomUsuario(nomUsuario: string):Observable<UserPagina>{
  return this.http.get<UserPagina>(this.url + 'getUsuarioNomUsuario.php?nomUsuario=' + nomUsuario);
}

 //login Usuarios y fundaciones
  onLoginUser(identidad: string, clave: string):Observable<any>{
    return this.http.post<UserPagAuth>(this.url + "loginUsuarioPag.php",
    {
      "identidad":identidad,
      "clave":clave
    },
