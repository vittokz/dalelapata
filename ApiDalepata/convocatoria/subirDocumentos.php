<?php
    ...
    try {
        if($file1upload_name!=""){
            if($tipoArchivo==0){
                $stmt 	= $pdo->query("INSERT INTO dlp_convocatorias(tipoArchivo,nombreArchivo,idMunicipio,estado,usuarioRegistro) 
                VALUES ('Formato carta de postulación','$file1upload_name','$municipio','Pendiente','$usuarioRegistro')");
            ...
            if($tipoArchivo==2){
                $stmt 	= $pdo->query("INSERT INTO dlp_convocatorias(tipoArchivo,nombreArchivo,idMunicipio,estado,usuarioRegistro) 
                VALUES ('Formato Propuesta','$file1upload_name','$municipio','Pendiente','$usuarioRegistro')");
                $data["resul"]= $stmt->rowCount();
            }
            if($tipoArchivo==3){
                $stmt 	= $pdo->query("INSERT INTO dlp_convocatorias(tipoArchivo,nombreArchivo,idMunicipio,estado,usuarioRegistro) 
                VALUES ('Formato Insumos','$file1upload_name','$municipio','Pendiente','$usuarioRegistro')");
                $data["resul"]= $stmt->rowCount();
             }
             if($tipoArchivo==4){
                $stmt 	= $pdo->query("INSERT INTO dlp_convocatorias(tipoArchivo,nombreArchivo,idMunicipio,estado,usuarioRegistro) 
                VALUES ('Cédula','$file1upload_name','$municipio','Pendiente','$usuarioRegistro')");
                $data["resul"]= $stmt->rowCount();
             }
             if($tipoArchivo==5){
                $stmt 	= $pdo->query("INSERT INTO dlp_convocatorias(tipoArchivo,nombreArchivo,idMunicipio,estado,usuarioRegistro) 
             ...
             if($tipoArchivo==6){
                $stmt 	= $pdo->query("INSERT INTO dlp_convocatorias(tipoArchivo,nombreArchivo,idMunicipio,estado,usuarioRegistro) 
                VALUES ('Formato Anexo2 Insumos Excel','$file1upload_name','$municipio','Pendiente','$usuarioRegistro')");
                $data["resul"]= $stmt->rowCount();
             }

             if($tipoArchivo==7){
                $stmt 	= $pdo->query("INSERT INTO dlp_convocatorias(tipoArchivo,nombreArchivo,idMunicipio,estado,usuarioRegistro) 
                VALUES ('Camara de comercio','$file1upload_name','$municipio','Pendiente','$usuarioRegistro')");
                $data["resul"]= $stmt->rowCount();
             }
