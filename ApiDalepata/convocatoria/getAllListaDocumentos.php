...
try {
   ...
   {
        $relacion = $row->id;
        $stmt2 	= $pdo->query("SELECT DISTINCT(idMunicipio),tipoArchivo,nombreArchivo,fechaRegistro,estado FROM dlp_convocatorias where idMunicipio like '$relacion' order by fechaRegistro DESC");
